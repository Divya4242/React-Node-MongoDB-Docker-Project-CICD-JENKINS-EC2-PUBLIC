    ...
    stages {
        ...
        stage('Transfer Frotend Build to EC2') {
            steps {
                ...
                // sh '''
                    rsync -avrx -e 'ssh -i ${PRIVATE_KEY} -o StrictHostKeyChecking=no' --delete --progress /var/lib/jenkins/workspace/frontend-backend-deploy/client/build ubuntu@65.0.87.100:/var/www                // '''    
                }
            }
        }
